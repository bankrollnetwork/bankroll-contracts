...
`deposit()` (an unguarded wrapper) checks `compoundClaimable()` and, at `AUTO_COMPOUND_MIN_USDC`
($100, ungoverned constant), runs `try this.compound() {} catch { emit AutoCompoundFailed(); }`,
then delegates to the guarded internal `_deposit()`. `compound()` is public, `nonReentrant`, and
UNINCENTIVIZED — anyone may call it (quiet-market safety valve) but earns nothing; 100% of every
harvest reinvests for holders.
...
| Direct deposit, ≥$100 claimable (compound fires) | 315,428 | 483,159 | +167k on the triggering deposit |
| Deposit right after a trigger | 221,538 | 237,943 | +16.4k (back to quiet cost) |
| Quiet zapDeposit | 382,816 | 395,920 | +13.1k |
| zapDeposit, ≥$100 claimable (compound fires) | 426,099 | 574,386 | +148k |
...
- Known tradeoff: with no incentive, compounding normally rides on deposit flow — quiet
  ...
  `AutoCompoundFailed` is the monitoring signal for a persistently reverting compound leg.
