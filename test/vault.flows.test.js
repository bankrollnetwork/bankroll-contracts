...
describe("VltUsdcVault — core flows", () => {
  ...
  describe("redeem", () => {
    it("returns BOTH tokens in kind, pro-rata, and burns the shares", async () => {
      ...
      const { amount0Out, amount1Out } = await getEvent(ctx.vault, rc, "Redeem");

      expect(amount0Out).to.be.greaterThan(0n);
      expect(amount1Out).to.be.greaterThan(0n); // in-kind: both sides returned
    ...
    it("previewRedeem matches the actual in-kind redeem amounts (minus V4 rounding)", async () => {
      ...
      const shares = await ctx.vault.balanceOf(ctx.alice.address);
      const [p0, p1] = await ctx.vault.previewRedeem(shares);
      expect(p0).to.be.greaterThan(0n);
      ...
      expect(amount0Out).to.be.greaterThanOrEqual((p0 * 99n) / 100n);
      expect(amount1Out).to.be.greaterThanOrEqual((p1 * 99n) / 100n);
    });
  ...
  describe("compound", () => {
    it("pays the finder, mints NO shares, and grows liquidity", async () => {
      ...
      const supplyBefore = await ctx.vault.totalSupply();
      const lBefore = await ctx.vault.positionLiquidity();
      const finderC0Before = await (ctx.usdcIsCurrency0 ? ctx.usdc : ctx.vlt).balanceOf(
        ctx.finder.address
      );
      const finderC1Before = await (ctx.usdcIsCurrency0 ? ctx.vlt : ctx.usdc).balanceOf(
        ctx.finder.address
      );

      const rc = await (await compound(ctx, ctx.finder)).wait();
      const { finder0, finder1, liquidityAdded } = await getEvent(ctx.vault, rc, "Compound");
      ...
      // Finder actually received the event-reported cut in each currency.
      const c0Token = ctx.usdcIsCurrency0 ? ctx.usdc : ctx.vlt;
      const c1Token = ctx.usdcIsCurrency0 ? ctx.vlt : ctx.usdc;
      expect((await c0Token.balanceOf(ctx.finder.address)) - finderC0Before).to.equal(finder0);
      expect((await c1Token.balanceOf(ctx.finder.address)) - finderC1Before).to.equal(finder1);
      expect(finder0 + finder1).to.be.greaterThan(0n);
    });
