...
APR, depositor activity, holders, pool price/volume, and per-wallet PnL.
...
| `02_fees_daily.sql` | Daily + cumulative realized fees (compound vs retained split; supply-side == fees, no cut) | **Yes** |
...
| `Compound(uint256,uint256,uint128)` | `0x9bc6dc46ca3a56321f65a8e41f17e5a16077d04974dd396b6af9d0bb91c62094` |
...
`usdcFees = varbinary_to_uint256(substr(data, 33, 32))`. `Compound` has no indexed args;
its `data` holds `vltFees, usdcFees, liquidityAdded`.
`Deposit` indexes `sender` (topic1) and `recipient` (topic2), with
...
- Fees identity: realized fees = Σ `Compound.fee` + Σ `FeesRetained.fee`; 100% of it is
  supply-side (no keeper, no finder cut); protocol take is structurally zero.
