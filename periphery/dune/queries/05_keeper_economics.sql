...
compounds AS (
  SELECT c.evt_block_time,
         ...
         c.finder,
         c.evt_tx_hash,
         CAST(c."vltFees" AS DOUBLE) / 1e18    AS fee_vlt,
         CAST(c."usdcFees" AS DOUBLE) / 1e6    AS fee_usdc,
         CAST(c."vltFinder" AS DOUBLE) / 1e18  AS finder_vlt,
         CAST(c."usdcFinder" AS DOUBLE) / 1e6  AS finder_usdc,
