...
--   supply-side   = the same total — 100% reinvests for shareholders (no keeper, no finder
--                   cut; compounding runs inside deposits); protocol take = 0.
...
--   topic table in ../README.md (Compound 0x9bc6dc46…, FeesRetained 0xbc53b087…).
...
all_fees AS (
  ...
  SELECT evt_block_time,
         ...
         'retained'                    -- harvested by deposit/redeem, retained for holders
...
SELECT
  ...
  -- supply-side == fees: 100% of every harvest reinvests for shareholders (no cut).
  COALESCE(d.fee_vlt, 0) * p.usdc_per_vlt + COALESCE(d.fee_usdc, 0)                    AS supply_side_usd,
